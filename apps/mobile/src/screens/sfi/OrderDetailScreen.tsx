...
export function OrderDetailScreen({ route, navigation }: Props) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {confirmed ? <Text style={styles.confirmed}>Order confirmed. Thank you for your purchase.</Text> : null}
            ...
            <View key={item.id} style={styles.itemRow}>
              <View style={styles.itemLeft}>
                {imageUrl ? (
                  ...
                  <View style={styles.imageFallback}><Text>🥚</Text></View>
                )}
                <View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.totalRow}><Text style={styles.totalLabel}>Subtotal</Text><Text>{formatPrice(Number(order.subtotal))}</Text></View>
        <View style={styles.totalRow}><Text style={styles.totalLabel}>Tax</Text><Text>{formatPrice(Number(order.tax))}</Text></View>
        <View style={styles.totalRow}><Text style={styles.totalLabel}>Shipping</Text><Text>{formatPrice(Number(order.shipping))}</Text></View>
        <View style={styles.totalRow}><Text style={styles.grandLabel}>Total</Text><Text style={styles.grandLabel}>{formatPrice(Number(order.total))}</Text></View>
      </View>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={styles.linkBtn} onPress={() => navigation.navigate('Products', {})}>
...
const styles = StyleSheet.create({
  ...
  confirmed: { backgroundColor: '#f0fdf4', borderColor: '#bbf7d0', borderWidth: 1, color: '#15803d', borderRadius: 8, padding: 10 },
  ...
  imageFallback: { width: 42, height: 42, backgroundColor: '#f9fafb', borderRadius: 6, alignItems: 'center', justifyContent: 'center' },
  ...
  linkBtn: { backgroundColor: '#f3f4f6', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 12 },
