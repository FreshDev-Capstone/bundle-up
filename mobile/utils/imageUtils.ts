...
export const getImageUrl = (imagePath: string): string => {
  ...
  if (imagePath.startsWith("http://") || imagePath.startsWith("https://")) {
    ...
    if (imagePath.includes("unsplash.com/photos/")) {
      ...
      const pattern1 = imagePath.match(/\/photos\/.*-([a-zA-Z0-9_-]{11})(?:\?|$)/);
      if (pattern1 && pattern1[1]) {
        ...
        if (pattern2 && pattern2[1]) {
          ...
          if (lastPart.length >= 10) { // Typical Unsplash ID length
            photoId = lastPart;
          } else {
            photoId = pattern2[1];
          }
  ...
  if (imagePath.startsWith("/assets/eggs/")) {
    ...
    const convertedPath = imagePath.replace("/assets/eggs/", "/assets/images/Eggs/");
